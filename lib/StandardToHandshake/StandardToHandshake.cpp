...
#include "StandardToHandshake/StandardToHandshake.h"
#include "IR/HandshakeOps.h"
  ...
  for (Block &block : f) {
    ...
    for (Operation &op : block) {
      if (op.getNumResults() > 0) {
        SmallVector<mlir::Value, 8> results(op.getResults());
...
void addForkOps(handshake::FuncOp f, ConversionPatternRewriter &rewriter) {
  for (Block &block : f) {
    for (Operation &op : block) {
...
void checkUseCount(Operation *op, Value res) {
  ...
  if (!res.hasOneUse())
    op->emitError("every result must have exactly one use");
  return;
}
  ...
  for (Block &block : f) {
    for (Operation &op : block) {
      if (auto callOp = dyn_cast<CallOp>(op)) {
        auto newInstanceOp = rewriter.create<handshake::InstanceOp>(
          callOp.getLoc(), callOp.getCallee(), callOp.getResultTypes(),
          callOp.getOperands());
        op.replaceUsesOfWith(op.getOperand(0), op.getResult(0));
      }
    }
  }
...
struct DFPass : public PassWrapper<DFPass, OperationPass<ModuleOp>> {
  void runOnOperation() override {
    ...
    // mlir::registerDialect<HandshakeOpsDialect>();
